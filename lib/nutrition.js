...
const weeklyTemplates = {
  ...
  pranzo: [
    { name: 'Pasta pollo e verdure', items: [['Pasta secca di semola (peso da cruda)', 85], ['Petto di pollo alla piastra', 150], ['Insalata mista', 180], ['Olio extravergine di oliva', 10]] },
    { name: 'Riso tonno e insalata', items: [['Riso bianco secco (peso da crudo)', 90], ['Tonno al naturale sgocciolato', 130], ['Insalata mista', 200], ['Olio extravergine di oliva', 10]] },
    { name: 'Ceci pane e verdure', items: [['Ceci cotti', 75], ['Pane comune', 55], ['Insalata mista', 200], ['Olio extravergine di oliva', 10]] }
  ],
  merenda: [
  ...
  cena: [
    { name: 'Salmone fresco patate e verdure', items: [['Salmone fresco', 150], ['Patate lesse', 260], ['Insalata mista', 200], ['Olio extravergine di oliva', 8]] },
    { name: 'Manzo pane e insalata', items: [['Manzo magro cotto', 150], ['Pane comune', 65], ['Insalata mista', 220], ['Olio extravergine di oliva', 8]] },
    { name: 'Lenticchie e verdure', items: [['Lenticchie cotte', 85], ['Pane comune', 45], ['Insalata mista', 220], ['Olio extravergine di oliva', 8]] }
  ]
};
...
export const workoutGoals = [
  { value: 'dimagrimento', label: 'Dimagrimento / bruciare calorie' },
  { value: 'tonificazione', label: 'Tonificazione generale' },
  { value: 'massa', label: 'Aumento massa muscolare' },
  { value: 'resistenza', label: 'Resistenza / fiato' },
  { value: 'salute', label: 'Salute, mobilità e ripartenza graduale' }
];
...
export const workoutLocations = [
  { value: 'chiuso', label: 'Al chiuso' },
  { value: 'aperto', label: 'All’aperto' },
  { value: 'entrambi', label: 'Entrambi' }
];

export const workoutEquipmentTypes = [
  { value: 'senza_attrezzi', label: 'Senza attrezzi' },
  { value: 'semplici', label: 'Attrezzi semplici' },
  { value: 'professionali', label: 'Attrezzi professionali / palestra' },
  { value: 'misto', label: 'Misto' }
];

const workoutDaysByCount = {
  1: [1],
  2: [1, 4],
  3: [1, 3, 5],
  4: [1, 2, 4, 5],
...
function workoutIntensity(level) {
  return {
    principiante: { sets: 2, reps: '10-12', rest: '75-90 sec', cardio: 18, label: 'facile/moderata, RPE 5-6/10' },
    intermedio: { sets: 3, reps: '10-15', rest: '60-75 sec', cardio: 25, label: 'moderata, RPE 6-7/10' },
    avanzato: { sets: 4, reps: '8-12', rest: '45-75 sec', cardio: 32, label: 'medio-alta, RPE 7-8/10' }
  }[level] || { sets: 2, reps: '10-12', rest: '75-90 sec', cardio: 18, label: 'facile/moderata, RPE 5-6/10' };
}

function equipmentKind(profile = {}) {
  const equipment = profile.workout_equipment || 'senza_attrezzi';
  if (equipment === 'professionali') return 'gym';
  if (equipment === 'semplici') return 'simple';
  if (equipment === 'misto') return 'mixed';
  return 'bodyweight';
}

function preferredPlace(profile = {}, index = 0) {
  const location = profile.workout_location || 'entrambi';
  if (location === 'aperto') return 'aperto';
  if (location === 'chiuso') return 'chiuso';
  return index % 2 === 0 ? 'chiuso' : 'aperto';
}

const exerciseLibraries = {
  bodyweight: {
    push: ['Piegamenti sulle braccia o inclinati', 'Dip su sedia', 'Shoulder tap', 'Plank con tocco spalla'],
    legs: ['Squat a corpo libero', 'Affondi alternati', 'Ponte glutei', 'Step-up su gradino'],
    pull: ['Rematore con asciugamano/zaino leggero', 'Superman a terra', 'Reverse snow angel', 'Prone Y-T-W'],
    core: ['Plank', 'Dead bug', 'Crunch controllato', 'Mountain climber lento'],
    cardio: ['Camminata veloce', 'Corsa leggera', 'Jumping jack basso impatto', 'Marcia sul posto']
  },
  simple: {
    push: ['Piegamenti sulle braccia', 'Military press con manubri/bottiglie', 'Chest press con elastico', 'Dip su sedia'],
    legs: ['Goblet squat con manubrio/zaino', 'Affondi con manubri', 'Stacco rumeno con manubri', 'Step-up su sedia stabile'],
    pull: ['Rematore con manubri/elastico', 'Pulley con elastico', 'Face pull con elastico', 'Curl bicipiti con manubri'],
    core: ['Plank', 'Dead bug', 'Russian twist leggero', 'Side plank'],
    cardio: ['Camminata veloce', 'Corda leggera o simulata', 'Cyclette se disponibile', 'Circuito basso impatto']
  },
  gym: {
    push: ['Chest press', 'Panca piana o manubri', 'Shoulder press', 'Push-down tricipiti'],
    legs: ['Leg press', 'Squat guidato o libero', 'Leg curl', 'Leg extension'],
    pull: ['Lat machine', 'Rematore al cavo', 'Pulley basso', 'Curl manubri/bilanciere'],
    core: ['Plank', 'Crunch machine o cable crunch', 'Pallof press', 'Side plank'],
    cardio: ['Tapis roulant camminata inclinata', 'Bike', 'Ellittica', 'Vogatore leggero']
  }
};

function pickFromLibrary(kind, category, index) {
  const primary = kind === 'mixed' ? (index % 2 === 0 ? 'simple' : 'gym') : kind;
  const library = exerciseLibraries[primary] || exerciseLibraries.bodyweight;
  const list = library[category] || exerciseLibraries.bodyweight[category] || [];
  return list[index % list.length] || '';
}

function workoutFocus(goal, index, totalDays) {
  if (goal === 'resistenza') return ['Cardio progressivo', 'Forza base', 'Intervalli cardio', 'Mobilità + core', 'Cardio lungo'][index % 5];
  if (goal === 'massa') return totalDays >= 4
    ? ['Upper body', 'Lower body', 'Riposo attivo/core', 'Upper body richiamo', 'Lower body richiamo', 'Full body'][index % 6]
    : ['Full body forza', 'Gambe + core', 'Parte alta + richiamo gambe'][index % 3];
  if (goal === 'tonificazione') return ['Full body', 'Gambe e glutei', 'Parte alta + core', 'Circuito metabolico', 'Mobilità + camminata'][index % 5];
  if (goal === 'salute') return ['Camminata + mobilità', 'Forza dolce full body', 'Core + postura', 'Cardio leggero', 'Mobilità generale'][index % 5];
  return ['Full body metabolico', 'Cardio moderato', 'Gambe + core', 'Parte alta + cardio', 'Camminata lunga'][index % 5];
}

function buildSessionExercises({ profile, dayIndex, totalDays }) {
  const goal = profile.workout_goal || 'dimagrimento';
  const level = profile.workout_level || 'principiante';
  const kind = equipmentKind(profile);
  const intensity = workoutIntensity(level);
  const place = preferredPlace(profile, dayIndex);
  const focus = workoutFocus(goal, dayIndex, totalDays);
  const minutes = Math.max(20, Math.min(90, Number(profile.workout_minutes || (level === 'principiante' ? 35 : level === 'intermedio' ? 45 : 55))));

  const push = pickFromLibrary(kind, 'push', dayIndex);
  const legs = pickFromLibrary(kind, 'legs', dayIndex + 1);
  const pull = pickFromLibrary(kind, 'pull', dayIndex + 2);
  const core = pickFromLibrary(kind, 'core', dayIndex + 3);
  const cardio = place === 'aperto'
    ? (dayIndex % 2 === 0 ? 'Camminata veloce all’aperto' : 'Corsa leggera/camminata a intervalli')
    : pickFromLibrary(kind, 'cardio', dayIndex);

  let lines = [];
  lines.push(`Durata indicativa: ${minutes} min`);
  lines.push(`Intensità: ${intensity.label}`);
  lines.push('');
  lines.push('Riscaldamento — 6/8 min');
  lines.push('- Mobilità anche/spalle + camminata leggera o cyclette');
  lines.push('');

  if (goal === 'resistenza' || focus.toLowerCase().includes('cardio')) {
    lines.push(`${focus}`);
    lines.push(`- ${cardio} — ${intensity.cardio}-${intensity.cardio + 10} min`);
    lines.push(`- ${legs} — ${Math.max(2, intensity.sets - 1)} serie x ${intensity.reps}`);
    lines.push(`- ${core} — ${Math.max(2, intensity.sets - 1)} serie x 30-45 sec`);
  } else if (goal === 'massa') {
    lines.push(`${focus}`);
    lines.push(`- ${legs} — ${intensity.sets} serie x ${level === 'avanzato' ? '6-10' : '8-12'} · recupero ${intensity.rest}`);
    lines.push(`- ${push} — ${intensity.sets} serie x ${level === 'avanzato' ? '6-10' : '8-12'} · recupero ${intensity.rest}`);
    lines.push(`- ${pull} — ${intensity.sets} serie x ${level === 'avanzato' ? '6-10' : '8-12'} · recupero ${intensity.rest}`);
    lines.push(`- ${core} — ${Math.max(2, intensity.sets - 1)} serie x 30-45 sec`);
    lines.push(`- Cardio leggero finale — 8/12 min`);
  } else {
    lines.push(`${focus}`);
    lines.push(`Esegui a circuito, recupero ${intensity.rest} tra i giri:`);
    lines.push(`- ${legs} — ${intensity.sets} x ${intensity.reps}`);
    lines.push(`- ${push} — ${intensity.sets} x ${intensity.reps}`);
    lines.push(`- ${pull} — ${intensity.sets} x ${intensity.reps}`);
    lines.push(`- ${core} — ${Math.max(2, intensity.sets - 1)} x 30-45 sec`);
    lines.push(`- ${cardio} — ${Math.max(10, intensity.cardio - 5)} min`);
  }

  lines.push('');
  lines.push('Defaticamento — 5 min');
  lines.push('- Stretching leggero di gambe, schiena, petto e spalle');
  lines.push('');
  lines.push('Regola pratica: se senti dolore articolare, capogiri o fiato anomalo, fermati e riduci intensità. Aumenta carichi o durata solo quando completi tutto con buona tecnica.');

  return { focus, place, minutes, exercises: lines.join('\n') };
}

export function buildGeneratedWorkoutWeek(profile = {}) {
  const rawDays = Number(profile.workout_days_per_week || 3);
  const daysCount = Math.max(1, Math.min(6, Math.round(rawDays)));
  const selectedDays = workoutDaysByCount[daysCount] || workoutDaysByCount[3];
  const goal = profile.workout_goal || 'dimagrimento';
  const level = profile.workout_level || 'principiante';
  const equipment = profile.workout_equipment || 'senza_attrezzi';
  const location = profile.workout_location || 'entrambi';

  const sessions = selectedDays.map((weekday, index) => {
    const session = buildSessionExercises({ profile: { ...profile, workout_goal: goal, workout_level: level, workout_equipment: equipment, workout_location: location }, dayIndex: index, totalDays: selectedDays.length });
    const dayLabel = weekdays.find(day => day.value === weekday)?.label || `Giorno ${index + 1}`;
    return {
      weekday,
      title: `${dayLabel} — ${session.focus}`,
      exercises: session.exercises,
      notes: [
        `Generato automaticamente in base a: ${daysCount} giorni/settimana, obiettivo ${goal}, livello ${level}, luogo ${location}, attrezzatura ${equipment}.`,
        profile.workout_notes ? `Note utente: ${profile.workout_notes}` : null
      ].filter(Boolean).join('\n'),
      source: 'generated',
  ...
  const weeklyMinutes = sessions.reduce((sum, item) => sum + Number(item.duration_minutes || 0), 0);
  const warnings = [];
  if (weeklyMinutes < 150 && goal !== 'massa') {
    warnings.push(`La scheda genera circa ${weeklyMinutes} minuti/settimana: per salute generale puoi aggiungere camminate leggere nei giorni liberi fino ad avvicinarti a 150 minuti settimanali.`);
  }
  if (daysCount < 2) {
    warnings.push('Con 1 solo giorno a settimana il programma è molto minimo: se puoi, aggiungi almeno una camminata o una seconda seduta leggera.');
  }

  return { sessions, warnings, weeklyMinutes };
}
